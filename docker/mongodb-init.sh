...
until mongo --host mongodb --eval "print(\"MongoDB est prêt\")"
do
...
db.deployments.insertMany([
  {
    model_id: db.models.findOne({name: "Prévision des ventes"})._id,
  ...
  {
    model_id: db.models.findOne({name: "Segmentation clients"})._id,
  ...
  {
    model_id: db.models.findOne({name: "Détection de fraude"})._id,
...
db.executions.insertMany([
  {
    deployment_id: db.deployments.findOne({name: "Déploiement Europe - Prévision des ventes"})._id,
    model_id: db.models.findOne({name: "Prévision des ventes"})._id,
    parameters: {
  ...
  {
    deployment_id: db.deployments.findOne({name: "Déploiement global - Segmentation clients"})._id,
    model_id: db.models.findOne({name: "Segmentation clients"})._id,
    parameters: {
  ...
  {
    deployment_id: db.deployments.findOne({name: "Déploiement Finance - Détection de fraude"})._id,
    model_id: db.models.findOne({name: "Détection de fraude"})._id,
    parameters: {
